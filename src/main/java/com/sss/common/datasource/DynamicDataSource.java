...
import com.sss.common.entity.SaasTenant;
...
//            setTargetDataSources(dynamicTargetDataSources2);// 将map赋值给父类的TargetDataSources
    ...
    public void createDataSourceWithCheck(SaasTenant masterTenant) throws Exception {
        String datasourceId = masterTenant.getTenantId();
    ...
    private  void createDataSource(SaasTenant masterTenant) throws Exception {
        String datasourceId = masterTenant.getTenantId();
        log.info("准备创建数据源"+datasourceId);
        String databasetype = masterTenant.getTenantId();
        String username = masterTenant.getUserName();
        String password = masterTenant.getPassword();
